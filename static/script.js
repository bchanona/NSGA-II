...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (missing.length > 0) {
    document.getElementById('status-bar').textContent = `❌ Error: Elementos faltantes: ${missing.join(', ')}`;
  }
});

// ── Paleta de tipos ────────────────────────────────────────────────
const TYPE_COLORS = {
  reel:     '#8A5200', image:    '#0D6640', carousel: '#1A3FAA',
  short:    '#A31840', video:    '#5B18A8', story:    '#C24A12',
};
const TYPE_BG = {
  reel:     '#FFF4D6', image:    '#E6FAF0', carousel: '#E8F0FF',
  short:    '#FFE8EF', video:    '#F0EAFF', story:    '#FFF0E8',
};

// ── Paleta de temas (sᵢ) ──────────────────────────────────────────
const THEME_COLORS = {
  educativo:      '#1A6B5A',
  entretenimiento:'#7B2F8E',
  promocional:    '#A84B00',
  inspiracional:  '#0E4FA8',
  tutorial:       '#7A5C00',
};
const THEME_BG = {
  educativo:      '#E0F5F0',
  entretenimiento:'#F5E8FF',
  promocional:    '#FFF0E0',
  inspiracional:  '#E6EFFF',
  tutorial:       '#FFF8DC',
};
const THEME_ICONS = {
  educativo:      '📚',
...
async function runOptimize() {
  ...
  const body = {
    n_posts:         +getVal('n_posts', 7),
    pop_size:        +getVal('pop_size', 60),
    generations:     +getVal('generations', 80),
    mutation_rate:   +getVal('mutation_rate', 0.3),
    hours_available: +getVal('hours_available', 10),
    platform:         getVal('platform', 'instagram'),
  };
...
async function loadEvolution() {
  ...
  evoChart = new Chart(ctx, {
    ...
    data: {
      labels: d.labels,
      datasets: [
        { label: 'Engagement', data: d.series.engagement, borderColor: '#FF6B35', backgroundColor: 'rgba(255,107,53,.08)', tension: .35, pointRadius: 0, borderWidth: 2.5 },
        { label: 'Alcance',    data: d.series.reach,      borderColor: '#4FACFE', backgroundColor: 'rgba(79,172,254,.08)', tension: .35, pointRadius: 0, borderWidth: 2.5 },
        { label: 'Retención',  data: d.series.retention,  borderColor: '#43E8A0', backgroundColor: 'rgba(67,232,160,.08)', tension: .35, pointRadius: 0, borderWidth: 2.5 },
        { label: 'Saturación', data: d.series.saturation, borderColor: '#F5C842', backgroundColor: 'rgba(245,200,66,.06)', tension: .35, pointRadius: 0, borderWidth: 1.5, borderDash: [5,4] },
      ],
    },
    options: {
      ...
      plugins: {
        legend: { position: 'top', labels: { boxWidth: 12, font: { size: 11, family: "'DM Sans', sans-serif" }, color: '#3B4A6B' } },
        tooltip: {
          backgroundColor: '#0D1B3E', titleColor: '#F5C842', bodyColor: '#E8EEF8',
          borderColor: 'rgba(255,255,255,.1)', borderWidth: 1,
          callbacks: { label: ctx => ` ${ctx.dataset.label}: ${ctx.parsed.y.toFixed(4)}` },
        },
      },
      scales: {
        x: { title: { display: true, text: 'Generación', font: { size: 11 }, color: '#8B9BBF' }, grid: { color: 'rgba(13,27,62,.06)' }, ticks: { color: '#8B9BBF', font: { size: 10 } } },
        y: { title: { display: true, text: 'Valor promedio (normalizado)', font: { size: 11 }, color: '#8B9BBF' }, grid: { color: 'rgba(13,27,62,.06)' }, ticks: { color: '#8B9BBF', font: { size: 10 } }, min: 0 },
      },
    },
  });
}
...
async function loadPareto() {
  ...
  paretoChart = new Chart(c1.getContext('2d'), {
    type: 'scatter',
    data: {
      datasets: [{ label: 'Solución Pareto',
        ...
        pointRadius: 6, pointHoverRadius: 10, borderWidth: 1.5,
      }],
    },
    options: {
      responsive: true, maintainAspectRatio: true,
      plugins: {
        ...
        tooltip: {
          ...tooltipBase,
          callbacks: { label: ctx => { const p = pts[ctx.dataIndex]; return [`Eng: ${p.engagement.toFixed(3)}`, `Reach: ${p.reach.toFixed(3)}`, `Ret: ${p.retention.toFixed(3)}`]; } },
        },
      },
      scales: {
  ...
  paretoChart2 = new Chart(c2.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...tooltipBase,
          callbacks: { label: ctx => { const p = pts[ctx.dataIndex]; return [`Eng: ${p.engagement.toFixed(3)}`, `Sat: ${p.saturation.toFixed(3)}`, `Prod: ${p.prod_time.toFixed(3)}`]; } },
        },
      },
      scales: {
        x: { ...scaleBase, title: { display: true, text: 'Engagement',  font: { size: 11 }, color: '#8B9BBF' } },
...
function renderSolution(idx) {
  ...
  // Estadísticas — incluye coherencia temática (sᵢ)
  document.getElementById('sol-metrics').innerHTML = `
    ...
    <span class="stat thm"><span class="stat-dot"></span>Coherencia sᵢ: ${(m.theme_coherence * 100).toFixed(1)}%</span>
  `;

  // Fila de secuencia temática cronológica
  const seqHtml = sol.theme_sequence
    ? `<div class="theme-seq">
         <span class="theme-seq-label">Secuencia temática (sᵢ):</span>
         ${sol.theme_sequence.map((t, i) => `
           <span class="theme-chip" style="background:${THEME_BG[t]};color:${THEME_COLORS[t]}">
             <span class="theme-pos">${i+1}</span>${THEME_ICONS[t]} ${t}
           </span>
           ${i < sol.theme_sequence.length - 1 ? '<span class="theme-arrow">→</span>' : ''}
         `).join('')}
       </div>`
    : '';

  document.getElementById('sol-theme-seq').innerHTML = seqHtml;

  // Tabla — columna Tema agregada
  document.getElementById('sol-body').innerHTML = sol.posts.map(p => `
    <tr>
      ...
      <td>
        <span class="theme-badge" style="background:${THEME_BG[p.theme]||'#F5F5F5'};color:${THEME_COLORS[p.theme]||'#555'}">
          ${THEME_ICONS[p.theme]||'📌'} ${p.theme_label||p.theme}
        </span>
      </td>
...
async function loadCalendar(rank) {
  ...
  // Secuencia temática del calendario
  const seqEl = document.getElementById('cal-theme-seq');
  if (seqEl && d.theme_sequence) {
    seqEl.innerHTML = `
      <div class="theme-seq cal-seq">
        <span class="theme-seq-label">Secuencia sᵢ del calendario:</span>
        ${d.theme_sequence.map((t, i) => `
          <span class="theme-chip" style="background:${THEME_BG[t]};color:${THEME_COLORS[t]}">
            <span class="theme-pos">${i+1}</span>${THEME_ICONS[t]} ${t}
          </span>
          ${i < d.theme_sequence.length - 1 ? '<span class="theme-arrow">→</span>' : ''}
        `).join('')}
        <span class="theme-coherence-badge">
          Coherencia: ${(d.metrics.theme_coherence * 100).toFixed(1)}%
        </span>
      </div>`;
  }
        ...
        ${day.posts.length
          ? day.posts.map(p => `
              <div class="post-pill"
                ...
                <div class="ptheme" style="color:${THEME_COLORS[p.theme]||'#666'};background:${THEME_BG[p.theme]||'#f5f5f5'}">
...
async function loadComparison() {
  ...
  const metrics = [
    ...
    { k: 'theme_coherence', label: 'Coherencia sᵢ',      higher: true,  color: '#A57BDB' },
  ];
